...
import { env } from 'bun'
import { getSemaphore as getSemaphoreDev } from '../index.ts'
import { getSemaphore as getSemaphoreDist } from '../dist/index.js'

let getSemaphore: typeof getSemaphoreDev
if (env.DIST) {
	console.log('using dist files')
	getSemaphore = getSemaphoreDist
} else {
	console.log('using dev files')
	getSemaphore = getSemaphoreDev
}
